...
class BSE:
    ...
    def readData(self):
        ...
        self.tz = array(f['parameters']['bilayer-model']['tz'])[0]
        ...
        for iwn,wn in enumerate(wnSet): # reduced tp frequencies !!
            ...
            for iK,K in enumerate(Kset):
                ...
                for k in kPatch:
                    kx = K[0]+k[0]; ky = K[1]+k[1]
                    ek = self.dispersion(kx,ky)
                    G0inv = (1j*wn+self.mu-self.U*(self.dens/4.-0.5))* np.identity(nOrb) - ek
                    G0 = linalg.inv(G0inv)
                    if (self.vertex_channel == "PARTICLE_PARTICLE_SUPERCONDUCTING"):
...
####### Momentum domain functions
    ...
    def dispersion(self,kx,ky):
        ...
        r1 = -2.*self.t*(cos(kx)+cos(ky)) - 4.0*self.tp*cos(kx)*cos(ky)
        ek[0,0] = r1
        ek[1,1] = r1
        ek[0,1] = -self.tz
        ek[1,0] = -self.tz
