...
class BSE:
    ...
    def readData(self):
        ...
        if self.oldFormat == False:
            ...
            self.U = array(f['parameters']['bilayer-Hubbard-model']['U'])[0]

            print("U = ",self.U)

            # self.tp1 = array(f['parameters']['bilayer-Hubbard-model']['t-prime1'])[0]

            self.tp1 = array(f['parameters']['bilayer-Hubbard-model']['t-prime'])[0]
            ...
            self.t1 = np.array(f['parameters']['bilayer-Hubbard-model']['t'])[0]
    ...
    def calcSus(self):
        ...
        for iqz in range(0,2):
            ...
            if self.vertex_channel in ("PARTICLE_PARTICLE_UP_DOWN","PARTICLE_PARTICLE_SUPERCONDUCTING"):
                ...
                print("spm   susceptibility for Qz = ",np.pi*iqz,": ",Ps)

                print("dwave susceptibility for Qz = ",np.pi*iqz,": ",Pd)
...
######### Plotting functions
    ...
    def calcKernelEigenValues(self):
        ...
        print ("Leading eigenvalues of lattice Bethe-salpeter equation for qz=0",self.lambdas[:,0])

        print ("Leading eigenvalues of lattice Bethe-salpeter equation for qz=pi",self.lambdas[:,1])
        ...
        if self.vertex_channel in ("PARTICLE_PARTICLE_SUPERCONDUCTING","PARTICLE_PARTICLE_UP_DOWN"):
            ...
            print ("10 leading eigenvalues of symmetrized Bethe-salpeter equation for qz=0",self.lambdas2[0:10])
