...
class BSE:
    ...
    def readData(self):
        ...
        GupRe = array(f['functions']['cluster_greens_function_G_k_w']['data'])[:,:,0,:,0,:,0]
        GupIm = array(f['functions']['cluster_greens_function_G_k_w']['data'])[:,:,0,:,0,:,1]
        GdnRe = array(f['functions']['cluster_greens_function_G_k_w']['data'])[:,:,1,:,1,:,0]
        GdnIm = array(f['functions']['cluster_greens_function_G_k_w']['data'])[:,:,1,:,1,:,1]
        ...
        self.Gup = GupRe + 1j * GupIm
        self.Gdn = GdnRe + 1j * GdnIm
        print ("Extracted Gup/Gdn.shape=", self.Gup.shape,'\n')
        ...
        self.sigmaup = s[:,:,0,:,0,:,0] + 1j *s[:,:,0,:,0,:,1]
        self.sigmadn = s[:,:,1,:,1,:,0] + 1j *s[:,:,1,:,1,:,1]
        ...
        self.nOrb = self.Gup.shape[2]
        self.NwG4 = self.G4.shape[0]
        self.Nc  = self.Gup.shape[1]
        self.NwG = self.Gup.shape[0]
        ...
        for iw in range(self.NwG):
            for ik in range(self.Nc):
                for b1 in range(self.nOrb):
                    for b2 in range(self.nOrb):
                        Gdiff = self.Gup[iw,ik,b1,b2]-self.Gdn[iw,ik,b1,b2]
                        if Gdiff>1.e-3:
                            print ("Gdn-Gdn) = ", Gdiff)
                                ...
                                for l4 in range(nOrb):
                                    ...
                                    if self.model=='threeband':
                                        if (l2 != l4 and l2 == 0) or (l2 != l4 and l4 == 0): 
                                            c1 = -self.Gup[iw1,ik,l3,l1] * self.Gdn[minusiwPlusiwm,ikPlusQ,l4,l2]
                                        else:
                                            c1 = self.Gup[iw1,ik,l3,l1] * self.Gdn[minusiwPlusiwm,ikPlusQ,l4,l2]
                                    else:
                                        c1 = self.Gup[iw1,ik,l3,l1] * self.Gdn[minusiwPlusiwm,ikPlusQ,l4,l2] 
                        ...
                        for l2 in range(nOrb):
                            for l3 in range(nOrb):
                                for l4 in range(nOrb):
                                    ...
                                    c1 = -self.Gup[iw1,ik,l1,l3] * self.Gdn[iwPlusiwm,ikPlusQ,l4,l2]
                                    self.chic0[iw,ik,l1,l2,iw,ik,l3,l4] = c1
                                    if (l1==l2) & (l3==l4):
                                        G4susQz0 += c1
                                        G4susQzPi += c1*exp(1j*np.pi*(l2-l3))
    ...
    def buildChi0Lattice(self,nkfine):
        ...
        self.sigmanegk_up = np.zeros((self.NwG,self.Nc,self.nOrb,self.nOrb),dtype='complex')
        self.sigmanegk_dn = np.zeros((self.NwG,self.Nc,self.nOrb,self.nOrb),dtype='complex')
        ...
        for l2 in range(self.nOrb):
            for l4 in range(self.nOrb):
                if (l2 != l4 and l2 == 0) or (l2 != l4 and l4 == 0): 
                    self.sigmanegk_up[:,:,l2,l4] = -self.sigmaup[:,:,l2,l4]
                    self.sigmanegk_dn[:,:,l2,l4] = -self.sigmadn[:,:,l2,l4]
                else:
                    self.sigmanegk_up[:,:,l2,l4] = self.sigmaup[:,:,l2,l4]
                    self.sigmanegk_dn[:,:,l2,l4] = self.sigmadn[:,:,l2,l4]
            ...
            for iK,K in enumerate(Kset):
                ...
                for k in kPatch:
                    ...
                    if (self.vertex_channel == "PARTICLE_PARTICLE_UP_DOWN"):
                        ...
                        G1inv = (1j*wn+self.mu) * np.identity(nOrb)-ek-self.sigmaup[iwG,iK,:,:]
                        G2inv = (-1j*wn+self.mu)* np.identity(nOrb)-emkpq-self.sigmanegk_dn[minusiwPlusiwm,iK,:,:]
                        ...
                        G1inv = (1j*wn+self.mu)*np.identity(nOrb)-ek-self.sigmaup[iwG,iK,:,:]
                        G2inv = (1j*wn+self.mu)*np.identity(nOrb)-ekpq-self.sigmanegk_dn[iwPlusiwm,iKQ,:,:]
                        G1 = linalg.inv(G1inv); G2 = -linalg.inv(G2inv)
    ...
    def calcKernelEigenValuesnew(self):
        ...
        if self.vertex_channel in ("PARTICLE_PARTICLE_SUPERCONDUCTING","PARTICLE_PARTICLE_UP_DOWN"):
            ...
            self.ind_d=0
            for ia in range(nt):
                r1 = dot(gk,self.evecs[iw0,:,0,0,ia]) * sum(self.evecs[:,1,0,0,ia])
                if abs(r1) >= 2.0e-1:
            ...
            self.ind_d=0
            for ia in range(nt):
                r1 = dot(gk,self.evecs[iw0,:,0,0,ia]) * sum(self.evecs[:,self.iKPi0,0,0,ia])
                if abs(r1) >= 2.0e-1:
                    self.lambdad = self.lambdas[ia]
                    self.ind_d = ia
                    self.found_d=True
