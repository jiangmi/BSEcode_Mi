...
class BSE:
    ...
    def readData(self):
        ...
        self.cluster = f["domains"]["CLUSTER"]["REAL_SPACE"]["super-basis"][:]
        ...
        self.sign = array(f['DCA-loop-functions']['sign'])
        print "sign:",self.sign,'\n'
        self.orbital=array(f['DCA-loop-functions']['orbital-occupancies'])
        ...
        self.sigmaarray=array(f['DCA-loop-functions']['L2_Sigma_difference'])
        ...
        for ver in self.vertex_channels:
            if 'G4_'+ver in f['functions'].keys():
                ...
                print "Vertex channel = ",self.vertex_channel,'\n'
                GG = array(f['functions']['G4_'+self.vertex_channel])
                print "original G4.shape=", GG.shape,'\n'
        
        self.G4 = array(f['functions']['G4_'+self.vertex_channel])[0,0,:,:,:,:,:,:,:,:]
        print "Extracted G4.shape=", self.G4.shape ,'\n'
        
        GG = array(f['functions']['cluster_greens_function_G_k_w'])
        print "original G.shape=", GG.shape,'\n'
        
        # Now read the cluster Green's function, only need spin up component
        self.Green = array(f['functions']['cluster_greens_function_G_k_w'])[:,:,0,:,0,:]
        print "Extracted G.shape=", self.Green.shape,'\n'
        #print("self.Green =",self.Green[512:1024,0,0,0])
        
        self.Greenkt = array(f['functions']['cluster_greens_function_G_k_t'])[:,:,0,:,0,:]
        #print("self.Greenkt =",self.Greenkt[120:137,0,0,0])
        self.Greenrt = array(f['functions']['cluster_greens_function_G_r_t'])[:,:,0,:,0,:]
        ...
        s = np.array(f['functions']['Self_Energy'])
        print "sigma.shape=", s.shape,'\n'
        self.sigmaoriginal = s[:,:,0,:,0,:] 
        ...
        self.Kvecs = array(f['domains']['CLUSTER']['MOMENTUM_SPACE']['elements'])
    ...
    def calcGamma_d(self):
        ...
        gkd = zeros(Nc)
        for iK in range(Nc):
            gkd[iK] = cos(self.Kvecs[iK][0]) - cos(self.Kvecs[iK][1])
    ...
    def AnalyzeEigvec_execute(self,evecs,lambdas,label):
        ...
        Nc=self.Nc; NwG4=self.NwG4; NwG=self.NwG; nt = self.nt; nOrb = self.nOrb
        gkd = zeros(Nc); gksx = zeros(Nc)
        for iK in range(Nc):
            gkd[iK]  = cos(self.Kvecs[iK][0]) - cos(self.Kvecs[iK][1])
            gksx[iK] = cos(self.Kvecs[iK][0]) + cos(self.Kvecs[iK][1])
     ...
     #               print real(self.evecs[iw0+iw,ic,io,io,0]), real(self.evecs[iw0-1-iw,ic,io,io,0])
        ...
        for io in range(nOrb):
            ...
            for ia in range(nt):
                ...
                r1 = dot(gkd,evecs[int(NwG4/2),:,io,io,ia]) * sum(evecs[:,self.iKPi0,io,io,ia])
                if abs(r1) >= 2.0e-1: 
       ...
       #                                   evecs[NwG4/2:NwG4,self.iK0Pi,io,io,ia])
        ...
        for io in range(nOrb):
            ...
            self.ind_d   =0
            for ia in range(nt):
                r1 = dot(gksx,evecs[int(self.NwG4/2),:,io,io,ia]) * sum(evecs[:,self.iKPi0,io,io,ia])
                if abs(r1) >= 2.0e-1: 
                    self.lambdad = lambdas[ia]
                    self.ind_d = ia
                    self.found_d=True
    ...
    def calcSCClusterSus(self):
        ...
        print (" ")
        print (" ")
        gkd = zeros(self.Nc); gksx = zeros(self.Nc)
        for iK in range(self.Nc):
            gkd[iK]  = cos(self.Kvecs[iK][0]) - cos(self.Kvecs[iK][1])
            gksx[iK] = cos(self.Kvecs[iK][0]) + cos(self.Kvecs[iK][1])
...
for T_ind, T in enumerate(Ts):
    ...
    for v in Nv:
        for q in qs:
            ...
            file_tp = './T='+str(Ts[T_ind])+'/dca_tp_mag_q'+str(v)+'.hdf5'
